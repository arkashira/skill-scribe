...
def test_add_task():
    ...
    task = RemediationTask(1, "Task 1", False)
...
def test_get_progress():
    tracker = RemediationTracker()
    task1 = RemediationTask(1, "Task 1", True)
    task2 = RemediationTask(2, "Task 2", False)
...
def test_get_notifications():
    ...
    tracker.add_task(task)
    tracker.update_task_status(1, True)
    assert len(tracker.get_notifications()) == 1

def test_save_to_database():
    tracker = RemediationTracker()
    task = RemediationTask(1, "Task 1", True)
    tracker.add_task(task)
    data = tracker.save_to_database()
    assert json.loads(data) == [{"id": 1, "name": "Task 1", "completion_status": True}]
